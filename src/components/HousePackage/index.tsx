...
*/
class HousePackage extends Component<Props, State> {
  state: State = {
    // 选中名称
    selectedNames: []
  }
  ...
  toggleSelect = (name: string) => {
    ...
    if (selectedNames.includes(name)) {
      ...
      newSelectedNames = selectedNames.filter((item: string) => item !== name)
    } else {
      // 未选中：添加到数组中
    ...
    // 传递给父组件
    (this.props as { onSelect: Function }).onSelect(newSelectedNames)
